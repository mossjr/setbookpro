...
export default function Layout({ children }: { children: ReactNode }) {
  const { isSidebarOpen, setSidebarOpen, desktopSidebarOpen, setDesktopSidebarOpen } =
    useAppStore();
  // Participants are locked to the host's song, so no library navigation.
  const isParticipant = useGigStore((s) => s.role === "participant");

  return (
    <div className="flex h-[100dvh] w-full overflow-hidden bg-background text-foreground">
      {/* Desktop Sidebar — collapsible so the song can fill the screen */}
      {!isParticipant && desktopSidebarOpen && (
      ...
      <Sheet open={isSidebarOpen && !isParticipant} onOpenChange={setSidebarOpen}>
