...
## Auto-fit multi-column song layout
The song body's "columns" display mode is now an **intelligent auto-fit** (`useAutoFitLayout.ts`): it picks BOTH the column count (1–6) and the font zoom (0.5–3) so the whole song is visible without scrolling at the largest readable font. It measures a hidden, off-screen `ChordRenderer` mirror rendered at zoom 1 — reading max-content width (shrink-wrapped, chord mode only) and the CSS-balanced `scrollHeight` per candidate column count — then derives the max zoom that fits both height and (chord-mode) column width, applies it, and refines against the real DOM over a few rAFs. `displayMode` is now `'scroll' | 'auto'`; legacy persisted `'columns'` is mapped to `'auto'` at read time (no migration). The computed columns/zoom are **ephemeral** (recomputed), never persisted; only `displayMode` and the manual scroll-mode `zoom` persist.

**Why:** A fixed 2-column layout wasted space and forced manual zooming. Director wanted "most economical with space" — auto is the smart default; plain `scroll` is the only other mode. Persisting an auto-computed zoom/column count would fight the recompute on every song/viewport change, so they're intentionally derived-only.

**How to apply:** The hidden measurer MUST stay rendered (abspos, visibility:hidden) only in auto mode, at zoom=1, with the same ChordRenderer props as the visible body — the optimizer reads from it, so removing it breaks fitting. Keep the manual-override guard (`manualRef`): manual +/- in auto mode pauses auto-recompute until the next song/mode change or Fit press. The 0.5 zoom / 6 column clamps are deliberate readability floors — extremely long songs may still overflow at min zoom (acceptable; user can switch to scroll). Auto-scroll forces `scroll` mode (`displayMode !== 'scroll'` check in `BottomScrollScrubber`) because multi-column can't vertical-scroll.
